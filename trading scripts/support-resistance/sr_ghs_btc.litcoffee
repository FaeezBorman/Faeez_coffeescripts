...
handle: (context, data)->
    ...
    if H > context.high
        for i of context.trade
            ...
            context.trade[i].csl(context.high,0.0001) # price has started to dipping, pulling in the the stoploss to 1$ above current price
            #debug context.trade[i].log()

    if H < context.high and instrument.price < H
        for i of context.trade
            #debug "detected price drop, adjusted all trade stop loss to 0.5% above #{instrument.price}"  
            context.trade[i].csl(H,0.0001) # price has started to dipping, pulling in the the stoploss to 1$ above current price
            #debug context.trade[i].log()
	
    if L >= context.low and instrument.price < L
            for i of context.trade
                #debug "detected price drop, adjusted all trade stop loss to 0.5% above #{instrument.price}"  
                context.trade[i].ctp(context.low,0.0001) # price has started to dipping, pulling in the the stoploss to 1$ above current price
                #debug context.trade[i].log()
    ...
    if instrument.price + 0.001 >= context.high and 
       ilow >= H and 
       diff_h_l > 0.001 and
       context.high >= H
       #then
        context.tf = 1 # confidence medium - 10%
    ...
    if context.tf > 0 
        ...
        context.trade[context.tradeNo].ctpsl(0.001,0.001)
